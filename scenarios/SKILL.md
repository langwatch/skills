...
Then drive everything via `langwatch scenario --help`, `langwatch suite --help` and `langwatch run-plan --help`. What follows is the surface as it actually is; `--help` is the live source when in doubt.
...
| **test suite** | A folder of scenarios: a name and the scenarios filed under it, and nothing else. Every project has a `Default` suite, so no scenario is loose. | `langwatch suite …` |
...
Running a test suite, and running a single scenario, are shorter forms of running a plan: the plan is named after the suite or the scenario and the target. Running is the only write; there is no separate save.

The UI calls the two surfaces **Agent Testing > Scenarios** (the suites and their scenarios) and **Agent Testing > Results** (the run plans, their runs, and the results of a run). There is no `langwatch simulation` command; results live under `langwatch simulation-run`.
...
- `--folder` files the scenario into a test suite, by name or by id. The suite must exist: create it with `langwatch suite create "<name>"` first, or leave the flag out and the scenario lands in `Default`. `langwatch scenario update <id> --folder "<suite>"` moves it later.
...
```bash
langwatch suite list --format json    # the suites, with the scenario count of each
langwatch suite get <id|name> --format json    # one suite and the scenarios in it
langwatch run-plan list --format json          # the plans the project already runs
```

Filing a scenario into a suite is `langwatch scenario update <id> --folder "<suite>"`. A scenario lives in exactly one suite, so this moves it rather than adding it to a second one.
...
```bash
langwatch suite run <suiteId|name> --target http:<agentId> --format json

langwatch suite run "Refund regression" \
  --target http:<agentId> --target prompt:<promptId> \
  --repeat 2 --note "after the refund policy change" --format json
```

- Every scenario in the suite runs against every target. The run count is `scenarios × targets × repeat`. Three scenarios × two targets × `--repeat 2` is twelve real LLM conversations. Say the number before launching anything large.
...
langwatch run-plan run --target http:<agentId> --suite "Refunds" --suite "Billing"
...
- Exactly one kind of scope per run: `--all`, or `--suite`, or `--label`, or `--scenario`. `--suite`, `--label` and `--scenario` repeat.
...
- Do NOT treat a test suite as a run configuration. A suite holds a name and its scenarios, nothing else: targets, repeat count and models belong to the run plan, and are given at run time
...
- Do NOT pass `--folder` a suite that does not exist. The command refuses it. Create the suite with `langwatch suite create "<name>"` first, or leave the flag out and let the scenario land in `Default`
- Do NOT mix scope flags on `run-plan run`. Exactly one of `--all`, `--suite`, `--label` or `--scenario` per run
- Do NOT choose the agent or prompt on the user's behalf, and do NOT decide for them between one scenario and the whole suite. Ask one short question and wait
